...
@click.option('--scryfall', '-s', is_flag=True, help="Use only scryfall.com (normally used as fallback)")
def import_rst(expansion, scryfall):
    ...
    for expansion in expansions:
        ...
        with open(f"{expansion}.rst", 'w') as output:
            try:
                if scryfall:
                    raise mtgsdk.restclient.MtgException('')
                cardset = mtgsdk.Set.find(expansion)

                starting_part = f""".. {cardset.name} (autogenerated)
.. include:: symbols.rst
        
:mtgexp:`{expansion}` {cardset.name}
{(len(cardset.name) + 11 + len(expansion)) * '='}
        
"""
                output.write(starting_part)
                while True:
                ...
                while True:
                    data = result.json()
                    output.write(f""".. {name} (autogenerated)
.. include:: symbols.rst

:mtgexp:`{expansion}` {name} (spoiler)
{(len(name) + len(expansion) + 22) * '='}
